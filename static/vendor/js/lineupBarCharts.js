...
function plotNbaGroupedBar(error, data) {
  if (error) throw error;

  var chartTitle = 'Scoring';
  var barGraphDiv = d3.select('#scoring_graph');
...
d3.csv("../static/nba-grouped-bar-data.csv", function(error, data) {
  plotNbaGroupedBar(error, data)
});
...
var barGraphDiv = d3.select('#ratings_graph');
var svgHeight = barGraphDiv.node().getBoundingClientRect().height;
var svgWidth = barGraphDiv.node().getBoundingClientRect().width;
...
svgGroup.selectAll("rect")
  ...
  SHOOTING GRAPH8
    ...
    var barGraphDiv = d3.select('#shooting_graph');
    var svgHeight = barGraphDiv.node().getBoundingClientRect().height;
    var svgWidth = barGraphDiv.node().getBoundingClientRect().width;
